...
**`RoboFlow` 의 경우 `Yolov5` 데이터셋과 `COCO` 형식 데이터셋 둘다 다운 받을 수 있기 때문에 두가지 다 함수를 구현하겠다.**

**먼저, `COCO` -> `Yolov5` 데이터셋의 형태로 변환하는 함수를 소개한다.**

**다음으로 `No COCO` -> `Yolov5` 데이터셋의 형태로 변환하는 함수도 소개한다.**

* **단, 이 함수(No COCO->Yolov5)**의 경우 **Yolov5형식으로 labeling** 되어있는 데이터를 사용한다고 가정함(예로 RoboFlow에서 Yolov5 데이터셋 다운받은 그런 상황을 가정할 수 있음)
* 왜냐하면 이 함수의 사용 목적은 **`Yolov5` 데이터셋에 이미지 resize와 labeiling의 클래스 분류 값 수정**만 하면 되기 때문이다.
...
* **예시**
  ...
  * `RoboFlow` 의 경우 `Yolo`가 아닌 `COCO`형태로 다운받을때 test, train, valid폴더 하웨 img들 및 json
...
* **카테고리 설정만 잘해뒀으면, `findCategory()` 함수로 자동으로 카테고리 매핑 값 반환**
...
# YOLO to YOLO - main2.py

이번 함수는 `No COCO(=Yolov5)` -> `Yolov5` 데이터셋의 형태로 변환하는 함수로써 YOLO 형태의 데이터셋에 **이미지 resize와 labeiling의 클래스 분류 값 커스텀**을 하는 함수이다.

* 즉, 라벨링(bbox 좌표 계산)을 안하는 점을 제외하고는 `main.py`와 비슷하다.
* 또한 `notUseImgFun()` 는 굳이 넣지 않았다.
* 만들 생각은 없었는데, json없이 YOLO -> YOLO 데이터 때 사용하려고 만든거라서 YOLO형태가 아닌건 직접 커스텀이 필요하다.

<br>

**따라서 해당 파일은 그냥 참고용으로만 사용하자.**

<br><br>

# Outro

**COCO 데이터를 YOLO 데이터 형태로 변환 및 이미지 리사이즈나 클래스명 커스텀 정도를 지원**
