...
def findCategory(category): # 사용 카테고리에 맞게 수정
    if(category==2 or category == 3): category = 0 # 종이
    elif(category==6): category = 1 # 플라스틱
    elif(category==4): category = 2 # 캔
...
def reImgLabFun(datas, cur, drawBBox):
    ...
    notUseImgs = {} # key : 0~500, val : inputDir
    ...
    for i in range(0, imgCnt):
        ...
        # 이미지 사용여부 결정
        for val in notUseImgs[int(fileName.split('.')[0])]:
            if(inputDir == val.replace('\n','')): 
                needImg = False
...
# (필요하다면) 선행 실행 함수
def notUseImgFun():
    with open('.\\notUseImgs.txt', 'w'): pass # 기존 txt 초기화
        ...
        for i in range(imgCnt):
            ...
            for j in range(bboxId, len(datas['annotations'])):
                ...
                if(imageId == image['id']):
                    ...
                    # 자신이 사용할 카테고리에 따라 커스텀
                    if(category==2 or category == 3 or 
                      category==6 or category == 4): 
                        useCategory = True
